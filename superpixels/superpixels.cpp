...
//            fclose(f);
    ...
    Mat initializeLabeling(string path, int mode = MODE_LABEL_MEDIAN)
    {
        ...
        for (int id=0; id < maxID+1; id++)
        {
            ...
            float l = (float) _arraySP[id].create_labelHist(_labelsInput,NUMLABELS,mode) / (float)NUMLABELS;
            _labels.setTo((float)l,_arraySP[id].getMask());
           /* printf("id %d label: %f\n",id,l);
            imshow("labels in",_labels*255);waitKey(0);*/
        }
