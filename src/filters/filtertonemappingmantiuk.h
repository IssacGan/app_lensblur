...
public:
	std::vector<std::string> propagatedValues() const override
		...
		return std::vector<std::string>{{
			std::string("Local brightness"),
			std::string("Local contrast")
		}};    
	...
	{	
		...
		cv::Mat b;
    		cv::exp(effect_local_brightness*(*local_brightness - cv::Scalar(0.5)), b);
		b*=(-brightness);
...
//		std::cerr<<"min = "<<min<<" - max = "<<max<<" - logmin = "<<logmin<<" - logmax = "<<logmax<<std::endl;
